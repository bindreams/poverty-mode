...
async fn resolve_version_reads_latest_version_txt_when_unpinned() {
    ...
    let v = tokio::task::spawn_blocking(move || poverty_mode::central::resolve_version_from(None, &base))
        .await
        .unwrap();
...
async fn resolve_version_prefers_explicit_pin_over_network() {
    ...
    let v = tokio::task::spawn_blocking(move || poverty_mode::central::resolve_version_from(Some("7.7.7"), &base))
        .await
        .unwrap();
