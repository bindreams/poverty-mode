...
async fn blocking_surface_is_spawn_blocking_safe() {
    ...
    let v =
        tokio::task::spawn_blocking(|| central::resolve_version_from(None, "http://127.0.0.1:1"))
            .await
            .unwrap();
    ...
    let info = tokio::task::spawn_blocking(|| {
        central::parse_wire_config(r#"{ "proxy_port": 4321, "proxy_secret": "s" }"#)
    })
    .await
    .unwrap()
    .unwrap();
    ...
    let state =
        tokio::task::spawn_blocking(|| central::classify_login_status(Some(0), "Logged in", ""))
            .await
            .unwrap();
    ...
    assert!(
        has_pin,
        "default-version linux/x86_64 pin must exist after M8.12"
    );
}
