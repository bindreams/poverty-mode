...
**Central.** When `central` is the (always-last) hop, the binary it runs is the `central.executable` setting (default `central`), resolved through `central::central_executable`. Central is external-only — poverty-mode never downloads it. The configured name reaches `Command` **unresolved**, so `execvp` performs the lookup; a `NotFound` spawn error becomes `central::missing_central_error`. Pre-resolving would be wrong: `Path::is_file()` is not executability, so a `chmod 000` file earlier on `PATH` would be selected where `execvp` skips it. `central::locate_executable` exists only for advisory reporting (`status` labels, `doctor` findings) and must never gate a run. poverty-mode assumes the user is logged in: it never runs `central config set`, never writes central's config (`~/.jetbrains-central/config.json`), and never drives login. `central.pinned_version` is a dead key kept only so an existing config still parses under `deny_unknown_fields`; it is never serialized back, and `doctor` warns when it is set.
