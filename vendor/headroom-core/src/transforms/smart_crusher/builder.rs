...
///
/// With the `embeddings` feature: `HybridScorer` (BM25 + ONNX embedding
/// fusion). In the trimmed build (no ONNX): pure-BM25 keyword scoring.
/// SmartCrusher's relevance gate still functions; cache-safety + shrink
/// behavior are unchanged.
fn default_scorer() -> Box<dyn RelevanceScorer + Send + Sync> {
    #[cfg(feature = "embeddings")]
    {
        Box::<HybridScorer>::default()
    }
    #[cfg(not(feature = "embeddings"))]
    {
        Box::<BM25Scorer>::default()
    }
}
...
impl SmartCrusherBuilder {
    ...
    pub fn with_default_oss_setup(self) -> Self {
        self.with_scorer(default_scorer())
            .add_default_oss_constraints()
    ...
    pub fn build(self) -> SmartCrusher {
        ...
        let scorer = self.scorer.unwrap_or_else(default_scorer);
