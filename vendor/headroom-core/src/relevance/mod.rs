...
pub fn create_scorer(tier: &str) -> Result<Box<dyn RelevanceScorer + Send + Sync>, String> {
    match tier.to_lowercase().as_str() {
        ...
        // Trimmed build: hybrid degrades to pure-BM25 keyword scoring
        // (no ONNX embeddings compiled). SmartCrusher's relevance gate
        // still functions; cache-safety + shrink behavior are unchanged.
        #[cfg(not(feature = "embeddings"))]
        "hybrid" => Ok(Box::new(BM25Scorer::default())),
        #[cfg(feature = "embeddings")]
        "embedding" => {
        ...
        #[cfg(not(feature = "embeddings"))]
        "embedding" => Err(
            "EmbeddingScorer not compiled (build headroom-core with the `embeddings` feature)"
                .to_string(),
        ),
        other => Err(format!(
