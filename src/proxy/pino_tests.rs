...
fn inject_skips_deferred_tools_places_on_last_non_deferred() {
    ...
    assert!(
        cc_of(&tools[2]).is_none(),
        "deferred tool must not be cached"
    );
    assert!(
        cc_of(&tools[3]).is_none(),
        "deferred tool must not be cached"
    );
}
...
fn inject_skips_tools_entirely_when_all_deferred() {
    ...
    let tools = body["tools"].as_array().unwrap();
    assert!(
        tools.iter().all(|t| cc_of(t).is_none()),
        "no deferred tool cached"
    );
}
...
fn inject_skips_tool_with_non_boolean_defer_loading() {
    ...
    assert!(
        ...
        "non-boolean defer_loading is not stamped"
    );
}
...
fn inject_skips_non_object_tool_element() {
    ...
    assert!(
        tools[1].as_object().is_none(),
        "non-object element left untouched"
    );
}
...
fn auto_cache_never_stamps_deferred_tool() {
    ...
    assert!(
        cc_of(&tools[1]).is_none(),
        "deferred tool never receives cache_control"
    );
    assert_eq!(
        cc_of(&tools[0]).unwrap(),
        &json!({ "type": "ephemeral", "ttl": "1h" }),
