...
fn overrides_precede_subcommand_and_user_args_preserved() {
    ...
    let argv = vec!["/opt/bin/codex".to_string(), "exec".to_string(), "--json".to_string()];
...
fn injects_self_contained_provider_pointing_at_base_url() {
    ...
    assert!(args.contains(&"model_providers.povertymode.base_url=\"http://127.0.0.1:4100/codex/openai\"".to_string()));
